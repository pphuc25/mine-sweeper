...
class MinesweeperAgentWeb(object):
    ...
    def get_loc(self):
        ...
        # print(pg.locateOnScreen(f'{IMGS}/{"beginner"}.png'))
        # for model in modes.keys():
        #     print(f'{IMGS}/{model}.png')
        #     print(pg.locateOnScreen(f'{IMGS}/{model}.png', confidence=LEVEL[model]))
        boards = {mode: pg.locateOnScreen(f'{IMGS}/{mode}.png', confidence=LEVEL[mode]) for mode in modes.keys()}
        print(boards)
    ...
    def get_state(self, board):
        ...
        print(state_im, self.nrows, self.ncols, 1)
        state_im = np.reshape(state_im[:self.nrows*self.ncols], (self.nrows, self.ncols, 1)).astype(object)
