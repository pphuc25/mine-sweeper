...
def parse_args():
    ...
    parser.add_argument('--save_model_every', type=int, default=1000,
                        help='Save model and replay every N episodes')
    parser.add_argument('--agg_stats_every', type=int, default=500,
                        help='Calculate stats every N episodes for tensorboard')
...
def main():
    ...
    for episode in tqdm(range(1, params.episodes+1), unit='episode'):
        ...
        if not episode % params.agg_stats_every:
            med_progress = round(np.median(progress_list[-params.agg_stats_every:]), 2)
            win_rate = round(np.sum(wins_list[-params.agg_stats_every:]) / params.agg_stats_every, 2)
            med_reward = round(np.median(ep_rewards[-params.agg_stats_every:]), 2)
        ...
        if not episode % params.save_model_every:
            with open(f'data/replay/{MODEL_NAME}.pkl', 'wb') as output:
